...
# path_py   = "/glade/work/glachat/python/antarctic/"
...
for yr in years:
    ...
    f_create = f"/glade/derecho/scratch/glachat/WB_climo_ERA5_py/WB_climo.{yr}.SH_v3.nc"
    data = Dataset(f"/glade/derecho/scratch/glachat/ERA5DTfield_{yr}_SH.nc")
    ...
    for time_step in np.arange(0,t):
            ...
            if matrix_LC1_cluster_mean.ndim == 2:  
                lat_c_LC1 = []
                lon_c_LC1 = []
                for evts in matrix_LC1_cluster_mean:
                    lat_c_LC1.append(evts[0])
                    lon_c_LC1.append(evts[1])
                lat_c_LC1_unique, preserved_order_lat_LC1, lat_c_LC1_counts = np.unique(lat_c_LC1,return_index=True,return_counts=True) 
                lon_c_LC1_unique, preserved_order_lon_LC1, lon_c_LC1_counts = np.unique(lon_c_LC1,return_index=True,return_counts=True)
                if np.logical_and(np.any(lat_c_LC1_counts>1,axis=0),np.any(lon_c_LC1_counts>1,axis=0)):
                    remove_idx_LC1 = np.argwhere(lat_c_LC1_counts>1)
                    matrix_LC1_cluster_mean = np.delete(matrix_LC1_cluster_mean,preserved_order_lat_LC1[remove_idx_LC1],axis=0)
                    # print(f'Repeat at {time_step} for LC1')
            ...
            elif np.logical_and(len(matrix_LC1_cluster_mean)>0,matrix_LC1_cluster_mean.ndim == 1):
                 matrix_LC1_cluster_mean_all.append(matrix_LC1_cluster_mean)

            if matrix_LC2_cluster_mean.ndim == 2:  
                lat_c_LC2 = []
                lon_c_LC2 = []
                for evts in matrix_LC2_cluster_mean:
                    lat_c_LC2.append(evts[0])
                    lon_c_LC2.append(evts[1])
                lat_c_LC2_unique, preserved_order_lat_LC2, lat_c_LC2_counts = np.unique(lat_c_LC2,return_index=True,return_counts=True) 
                lon_c_LC2_unique, preserved_order_lon_LC2, lon_c_LC2_counts = np.unique(lon_c_LC2,return_index=True,return_counts=True)
                if np.logical_and(np.any(lat_c_LC2_counts>1,axis=0),np.any(lon_c_LC2_counts>1,axis=0)):
                    remove_idx_LC2 = np.argwhere(lat_c_LC2_counts>1)
                    matrix_LC2_cluster_mean = np.delete(matrix_LC2_cluster_mean,preserved_order_lat_LC2[remove_idx_LC2],axis=0)
                    # print(f'Repeat at {time_step} for LC2')
