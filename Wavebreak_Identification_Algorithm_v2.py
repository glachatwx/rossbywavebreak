...
for yr in years:
    ...
    f_create = f"/glade/derecho/scratch/glachat/WB_climo_ERA5_py/WB_climo.{yr}.SH_v2.nc"
    ...
    for time_step in np.arange(0,t):
            ...
            # For time steps with a single event, no need to check for repeat events
                 ...
                 matrix_LC1_cluster_mean_all.append(matrix_LC1_cluster_mean)
            # Run the same check formatrix_LC2_cluster_mean_allmatrix_LC2_cluster_mean_all repeat events for LC2
            if  matrix_LC2_cluster_mean.ndim == 2:
                # Make sure to add the time step of the identified wave break 
                # There is a very rare possibility that there will be repeat events included, so ensure that these events are not captured
                for indx_del_LC2, LC2_evts in enumerate(matrix_LC2_cluster_mean):
